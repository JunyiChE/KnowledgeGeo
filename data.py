...
class DataLoader():
    def __init__(self, data_home, bucket_size=50, encoding='utf-8', 
                 celebrity_threshold=10, one_hot_labels=True, mindf=10, maxdf=0.2,
    ...
    def load_data(self):
        ...
        df_train.drop_duplicates(['user'], inplace=True, keep='first')  # Keep the first occurrence
        df_train.set_index('user', inplace=True)

        df_dev['user'] = df_dev['user'].apply(lambda x: str(x).lower())
        df_dev.drop_duplicates(['user'], inplace=True, keep='first')  # Keep the first occurrence
        df_dev.set_index('user', inplace=True)

        df_test['user'] = df_test['user'].apply(lambda x: str(x).lower())
        df_test.drop_duplicates(['user'], inplace=True, keep='first')  # Keep the first occurrence
        df_test.set_index('user', inplace=True)
    ...
    #     # Load city mappings from CSV files
    #     train_df = pd.read_csv(os.path.join(self.data_home, 'training_list.csv'))
    #     dev_df = pd.read_csv(os.path.join(self.data_home, 'dev_list.csv'))
    #     test_df = pd.read_csv(os.path.join(self.data_home, 'test_list.csv'))
    #
    #     # Combine all data to create a unique mapping of cities to labels
    #     combined_df = pd.concat([train_df, dev_df, test_df])
    #     unique_cities = combined_df[['Nearest_City', 'City_Latitude', 'City_Longitude']].drop_duplicates()
    #
    #     # Create a dictionary for city to label mapping and a list for city coordinates
    #     city_to_label = {}
    #     cities = []
    #     for i, row in unique_cities.iterrows():
    #         city = row['Nearest_City']
    #         if city not in city_to_label:
    #             city_to_label[city] = len(city_to_label)
    #             cities.append((row['City_Latitude'], row['City_Longitude']))
    #
    #     # Assign classes based on city mappings
    #     self.train_classes = train_df['Nearest_City'].map(city_to_label).values
    #     self.dev_classes = dev_df['Nearest_City'].map(city_to_label).values
    #     self.test_classes = test_df['Nearest_City'].map(city_to_label).values
    #
    #     # Convert to one-hot encoding if necessary
    #     if self.one_hot_labels:
    #         num_labels = len(city_to_label)
    #         self.train_classes = self.to_one_hot(self.train_classes, num_labels)
    #         self.dev_classes = self.to_one_hot(self.dev_classes, num_labels)
    #         self.test_classes = self.to_one_hot(self.test_classes, num_labels)
    #
    #     # Save city information
    #     self.city_info = cities
    def cluster_cities(self,df, threshold_miles):
        # Convert miles to kilometers
        threshold_km = threshold_miles * 1.60934

        # Create a DBSCAN clustering instance
        db = DBSCAN(eps=threshold_km, min_samples=1, metric=lambda a, b: haversine(a, b, unit='km'))

        # Fit the model
        coords = df[['City_Latitude', 'City_Longitude']].values
        db.fit(coords)
    ...
    def assignClasses(self):
        ...
        # Cluster cities in the training set
        train_cities = train_df[['Nearest_City', 'City_Latitude', 'City_Longitude']].drop_duplicates()
        clustered_cities = self.cluster_cities(train_cities, 1)
        city_to_cluster_label = dict(zip(train_cities['Nearest_City'], clustered_cities['cluster_label']))

        # Extract first city coordinates for each cluster and update city_info
        city_info_df = clustered_cities.groupby('cluster_label').first()[['City_Latitude', 'City_Longitude']]
        self.city_info = [(row['City_Latitude'], row['City_Longitude']) for _, row in city_info_df.iterrows()]

        # Create a mapping from cluster label to index in city_info
        cluster_label_to_index = {label: idx for idx, label in enumerate(city_info_df.index)}

        # Function to map cities to indices in city_info
        def map_to_city_info_index(city):
            cluster_label = city_to_cluster_label.get(city, -1)  # default label -1 if city is not found
            return cluster_label_to_index.get(cluster_label, -1)

        # Assign indices based on city mappings
        self.train_classes = train_df['Nearest_City'].map(map_to_city_info_index).values
        self.dev_classes = dev_df['Nearest_City'].map(map_to_city_info_index).values
        self.test_classes = test_df['Nearest_City'].map(map_to_city_info_index).values

        # Convert to one-hot encoding if necessary
        if self.one_hot_labels:
            num_labels = len(self.city_info)
            self.train_classes = self.to_one_hot(self.train_classes, num_labels)
            self.dev_classes = self.to_one_hot(self.dev_classes, num_labels)
            self.test_classes = self.to_one_hot(self.test_classes, num_labels)

    def to_one_hot(self, classes, num_classes):
        one_hot = np.zeros((len(classes), num_classes), dtype=np.float32)
        one_hot[np.arange(len(classes)), classes] = 1
        return one_hot
    ...
    def dump_all(self, filename):
        ...
        data_to_dump = {
            'user_info': {
                'train': self.df_train[['lat', 'lon']].to_dict('records'),
                'dev': self.df_dev[['lat', 'lon']].to_dict('records'),
                'test': self.df_test[['lat', 'lon']].to_dict('records')
            },
            'y': {
                'train': self.train_classes.tolist() if isinstance(self.train_classes,
                                                                   np.ndarray) else self.train_classes,
                'dev': self.dev_classes.tolist() if isinstance(self.dev_classes, np.ndarray) else self.dev_classes,
                'test': self.test_classes.tolist() if isinstance(self.test_classes, np.ndarray) else self.test_classes
            },
            'x': {
                'train': self.X_train,
                'dev': self.X_dev,
            ...
            'adjacency_matrix': nx.to_scipy_sparse_matrix(self.graph)
        }

        # Dump the data
        logging.info("Dump the data............" )
        dump_obj(data_to_dump, filename)
...
if __name__ == '__main__':
    ...
    data_loader.dump_all('dumped_data.pkl')
    pdb.set_trace()
